...
public class ResumeTestData {
    ...
    public static void main() {
        ...
        Position position = new Position(startDate, endDate, "Автор проекта", "Создание, организация и проведение Java онлайн проектов и стажировок");
        Organization organization = new Organization(homePage, Collections.singletonList(position));
        ...
        position = new Position(startDate, endDate, "Старший разработчик (backend)", "Проектирование и разработка онлайн платформы управления проектами Wrike (Java 8 API, Maven, Spring, MyBatis, Guava, Vaadin, PostgreSQL, Redis). Двухфакторная аутентификация, авторизация по OAuth1, OAuth2, JWT SSO");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemExperience.add(organization);

        homePage = new Link("RIT Center", "");
        startDate = LocalDate.of(2012, APRIL, 1);
        endDate = LocalDate.of(2014, OCTOBER, 1);
        position = new Position(startDate, endDate, "Java архитектор", "Организация процесса разработки системы ERP для разных окружений: релизная политика, версионирование, ведение CI (Jenkins), миграция базы (кастомизация Flyway), конфигурирование системы (pgBoucer, Nginx), AAA via SSO. Архитектура БД и серверной части системы. Разработка интергационных сервисов: CMIS, BPMN2, 1C (WebServices), сервисов общего назначения (почта, экспорт в pdf, doc, html). Интеграция Alfresco JLAN для online редактирование из браузера документов MS Office. Maven + plugin development, Ant, Apache Commons, Spring security, Spring MVC, Tomcat,WSO2, xcmis, OpenCmis, Bonita, Python scripting, Unix shell remote scripting via ssh tunnels, PL/Python");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemExperience.add(organization);
        ...
        position = new Position(startDate, endDate, "Ведущий программист", "Участие в проекте Deutsche Bank CRM (WebLogic, Hibernate, Spring, Spring MVC, SmartGWT, GWT, Jasper, Oracle). Реализация клиентской и серверной части CRM. Реализация RIA-приложения для администрирования, мониторинга и анализа результатов в области алгоритмического трейдинга. JPA, Spring, Spring-MVC, GWT, ExtGWT (GXT), Highstock, Commet, HTML5");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemExperience.add(organization);
        ...
        position = new Position(startDate, endDate, "Ведущий специалист", "Дизайн и имплементация Java EE фреймворка для отдела \"Платежные Системы\" (GlassFish v2.1, v3, OC4J, EJB3, JAX-WS RI 2.1, Servlet 2.4, JSP, JMX, JMS, Maven2). Реализация администрирования, статистики и мониторинга фреймворка. Разработка online JMX клиента (Python/ Jython, Django, ExtJS)");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemExperience.add(organization);
        ...
        position = new Position(startDate, endDate, "Разработчик ПО", "Реализация клиентской (Eclipse RCP) и серверной (JBoss 4.2, Hibernate 3.0, Tomcat, JMS) частей кластерного J2EE приложения (OLAP, Data mining)");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemExperience.add(organization);
        ...
        position = new Position(startDate, endDate, "Разработчик ПО", "Разработка информационной модели, проектирование интерфейсов, реализация и отладка ПО на мобильной IN платформе Siemens @vantage (Java, Unix)");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemExperience.add(organization);
        ...
        position = new Position(startDate, endDate, "Инженер по аппаратному и программному тестированию", "Тестирование, отладка, внедрение ПО цифровой телефонной станции Alcatel 1000 S12 (CHILL, ASM)");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemExperience.add(organization);
        ...
        position = new Position(startDate, endDate, "\"Functional Programming Principles in Scala\" by Martin Odersky", "");
        organization = new Organization(homePage, Collections.singletonList(position));
        ...
        position = new Position(startDate, endDate, "Курс \"Объектно-ориентированный анализ ИС. Концептуальное моделирование на UML.\"", "");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemEducation.add(organization);
        ...
        position = new Position(startDate, endDate, "3 месяца обучения мобильным IN сетям (Берлин)", "");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemEducation.add(organization);
        ...
        position = new Position(startDate, endDate, "6 месяцев обучения цифровым телефонным сетям (Москва)", "");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemEducation.add(organization);
        ...
        Position period1 = new Position(startDate, endDate, "Аспирантура (программист С, С++)", "");
        startDate = LocalDate.of(1987, SEPTEMBER, 1);
        endDate = LocalDate.of(1993, JULY, 1);
        Position period2 = new Position(startDate, endDate, "Инженер (программист Fortran, C)", "");
        ...
        position = new Position(startDate, endDate, "Закончил с отличием", "");
        organization = new Organization(homePage, Collections.singletonList(position));
        itemEducation.add(organization);
